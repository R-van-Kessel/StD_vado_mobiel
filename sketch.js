...
function setup() {
    ...
    // BUTTONS
    let btnScale = IS_MOBILE ? (window.innerWidth / 1100) : 1; // Schaal buttons evenredig
    
    checkButton = createButton('Nakijken');
    checkButton.parent(container);
    checkButton.mousePressed(checkAnswers);
    styleButton(checkButton, '#4CAF50', '8px 12px');
    checkButton.position((MARGIN + BUTTON_1_X) * btnScale, BUTTON_Y);
    if (IS_MOBILE) checkButton.style('font-size', '14px');
    
    scoreButton = createButton('Score: 0/10');
    scoreButton.parent(container);
    scoreButton.mousePressed(showScoreFeedback);
    styleButton(scoreButton, '#9C27B0', '8px 12px');
    scoreButton.style('height', '38px');
    scoreButton.style('min-height', '38px');
    scoreButton.position((MARGIN + BUTTON_2_X) * btnScale, BUTTON_Y);
    if (IS_MOBILE) scoreButton.style('font-size', '14px');
    
    resetButton = createButton('Reset');
    resetButton.parent(container);
    resetButton.mousePressed(resetGame);
    styleButton(resetButton, '#f44336', '8px 12px');
    resetButton.position((MARGIN + BUTTON_3_X) * btnScale, BUTTON_Y);
    if (IS_MOBILE) resetButton.style('font-size', '14px');
    
    infoButton = createButton('ℹ Info');
    infoButton.parent(container);
    infoButton.mousePressed(showInfo);
    styleButton(infoButton, '#03A9F4', '8px 12px');
    infoButton.position((MARGIN + BUTTON_4_X) * btnScale, BUTTON_Y);
    if (IS_MOBILE) infoButton.style('font-size', '14px');
