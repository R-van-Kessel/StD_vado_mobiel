...
function gridY(row) {
  return MARGIN + row * CELL_SIZE + TITLE_SPACE;
}
...
function gridX(col) {
  return (MARGIN + col * CELL_SIZE) * CANVAS_SCALE;
}

function gridY(row) {
  return (MARGIN + row * CELL_SIZE + TITLE_SPACE) * CANVAS_SCALE;
}
...
function setup() {
  ...
  if (IS_MOBILE) {
  const realWidth = COLS * CELL_SIZE + MARGIN * 2;
  CANVAS_SCALE = window.innerWidth / realWidth;

  CANVAS_SCALE = min(CANVAS_SCALE, 1); // nooit groter dan desktop
} else {
  CANVAS_SCALE = 1;
}
...
const btnW = (CELL_SIZE - 20) * CANVAS_SCALE;
const btnH = BUTTON_HEIGHT * CANVAS_SCALE;
const btnY = gridY(0) + (CELL_SIZE * CANVAS_SCALE - btnH) / 2;
  
checkButton = createButton('Nakijken');
checkButton.position(gridX(0) + 10 * CANVAS_SCALE, btnY);
checkButton.size(btnW, btnH);
checkButton.mousePressed(checkAnswers);

scoreButton = createButton('Score: 0/10');
scoreButton.position(gridX(1) + 10 * CANVAS_SCALE, btnY);
scoreButton.size(btnW, btnH);
scoreButton.mousePressed(showScoreFeedback);

resetButton = createButton('Reset');
resetButton.position(gridX(2) + 10 * CANVAS_SCALE, btnY);
resetButton.size(btnW, btnH);
resetButton.mousePressed(resetGame);

infoButton = createButton('ℹ Info');
infoButton.position(gridX(3) + 10 * CANVAS_SCALE, btnY);
infoButton.size(btnW, btnH);
