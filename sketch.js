...
function setup() {
    // Detecteer mobiel NU (in setup, niet bij page load)
    IS_MOBILE = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ||
                window.innerWidth < 768;
    SCALE_FACTOR = IS_MOBILE ? 0.5 : 1; // Nog kleinere schaal voor mobiel
    
    console.log("Setup - IS_MOBILE:", IS_MOBILE);
    console.log("Setup - Window width:", window.innerWidth);
    console.log("Setup - SCALE_FACTOR:", SCALE_FACTOR);
    ...
    // CSS transform schaling voor mobiel
    if (IS_MOBILE) {
        cnv.elt.style.transform = `scale(${SCALE_FACTOR})`;
        cnv.elt.style.transformOrigin = 'top left';
        container.elt.style.height = (canvasHeight * SCALE_FACTOR + 100) + 'px';
        
        // Pas button posities aan voor mobiel
        let buttonScale = SCALE_FACTOR;
        checkButton.style('transform', `scale(${buttonScale})`);
        scoreButton.style('transform', `scale(${buttonScale})`);
        resetButton.style('transform', `scale(${buttonScale})`);
        infoButton.style('transform', `scale(${buttonScale})`);
    }
  ...
  function draw() {
    ...
    console.log("Draw wordt aangeroepen");
...
function pointerDown(px, py) {
  ...
  // Compenseer voor schaling op mobiel
  if (IS_MOBILE) {
    let bounds = document.querySelector('canvas').getBoundingClientRect();
    px = (px - bounds.left) / SCALE_FACTOR;
    py = (py - bounds.top) / SCALE_FACTOR;
  }
