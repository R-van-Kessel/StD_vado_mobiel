...
function generateQuestions() {
    ...
    // Score button update gebeurt nu automatisch in draw()
    // Geen HTML button meer
...
window.addEventListener('resize', () => {
    if (IS_MOBILE) {
        ...
        console.log("Resize - New SCALE_FACTOR:", SCALE_FACTOR);
        console.log("Resize - Screen:", window.innerWidth, "x", window.innerHeight);
        
        // Pas canvas CSS aan
        let cnv = document.querySelector('canvas');
        if (cnv) {
            // Force refresh van de styling
            cnv.style.maxWidth = '100vw';
            cnv.style.width = '100%';
            cnv.style.height = 'auto';
        }
        
        // Herpositioneer buttons
        if (canvasButtons.length > 0) {
            let btnW = 75;
            let btnH = 35;
            let btnY = MARGIN + TITLE_SPACE + BUTTON_HEIGHT + 30;
            let btnGap = 15;
            ...
            canvasButtons[0].x = startX;
            canvasButtons[0].y = btnY;
            canvasButtons[0].w = btnW;
            canvasButtons[0].h = btnH;
            
            canvasButtons[1].x = startX + btnW + btnGap;
            canvasButtons[1].y = btnY;
            canvasButtons[1].w = btnW + 20;
            canvasButtons[1].h = btnH;
            
            canvasButtons[2].x = startX + (btnW + btnGap) * 2 + 20;
            canvasButtons[2].y = btnY;
            canvasButtons[2].w = btnW;
            canvasButtons[2].h = btnH;
            
            canvasButtons[3].x = startX + (btnW + btnGap) * 3 + 20;
            canvasButtons[3].y = btnY;
            canvasButtons[3].w = btnW;
            canvasButtons[3].h = btnH;
        }
    }
});
