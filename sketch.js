...
function setup() {
    ...
    const buttonY = gridY(BUTTON_ROW) + (CELL_SIZE - BUTTON_HEIGHT) / 2;

checkButton = createButton('Nakijken');
checkButton.position(gridX(0) + 10, buttonY);
checkButton.size(CELL_SIZE - 20, BUTTON_HEIGHT);
checkButton.mousePressed(checkAnswers);

scoreButton = createButton('Score: 0/10');
scoreButton.position(gridX(1) + 10, buttonY);
scoreButton.size(CELL_SIZE - 20, BUTTON_HEIGHT);
scoreButton.mousePressed(showScoreFeedback);

resetButton = createButton('Reset');
resetButton.position(gridX(2) + 10, buttonY);
resetButton.size(CELL_SIZE - 20, BUTTON_HEIGHT);
resetButton.mousePressed(resetGame);

infoButton = createButton('ℹ Info');
infoButton.position(gridX(3) + 10, buttonY);
infoButton.size(CELL_SIZE - 20, BUTTON_HEIGHT);
