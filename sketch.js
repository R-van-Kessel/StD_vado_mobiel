/* ======================
   GLOBAL STYLE
====================== */
console.log("Script begint te laden...");
...
console.log("Error handler geïnstalleerd");
/* ===== DEVICE DETECT & SCALE (ADD AT TOP) ===== */

let IS_MOBILE = true; // Forceer mobiel voor test
let SCALE_FACTOR = 1;

console.log("Variabelen gedeclareerd");
...
function generateQuestions() {
  ...
  // ===== MAAK VRAAG BLOKJES (rijen 1 en 2) - rij 0 is voor buttons =====
    let questionIndex = 0;
    for (let row = 1; row < 3; row++) {  // ← Was 0 tot 2, nu 1 tot 3
    ...
    let answerIndex = 0;
    for (let row = 3; row < 5; row++) {  // ← Was 2 tot 4, nu 3 tot 5
        for (let col = 0; col < COLS; col++) {
            blocks.push({
                col: col,
                row: row,
...
function setup() {
    ...
    // Detecteer mobiel in setup
    IS_MOBILE = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
    SCALE_FACTOR = 1;
    
    console.log("Setup - IS_MOBILE:", IS_MOBILE);
    console.log("Setup - Window width:", window.innerWidth);
    ...
    // Button container (absoluut gepositioneerd over canvas in rij 0)
    let buttonContainer = createDiv();
    buttonContainer.parent(container);
    buttonContainer.style('display', 'flex');
    buttonContainer.style('gap', IS_MOBILE ? '3px' : '10px');
    buttonContainer.style('justify-content', 'center');
    buttonContainer.style('flex-wrap', 'wrap');
    buttonContainer.style('position', 'absolute');
    
    // Bereken positie als percentage van canvas hoogte
    let btnTopPercent = ((MARGIN + TITLE_SPACE + BUTTON_HEIGHT + 20) / canvasHeight) * 100;
    buttonContainer.style('top', btnTopPercent + '%');
    
    buttonContainer.style('left', '50%');
    buttonContainer.style('transform', 'translateX(-50%)');
    buttonContainer.style('z-index', '100');
    buttonContainer.style('width', IS_MOBILE ? '95%' : 'auto');
    buttonContainer.style('max-width', '100%');
   
    let btnPadding = IS_MOBILE ? '3px 4px' : '10px 15px';
    let btnFontSize = IS_MOBILE ? '9px' : '16px';
    ...
    checkButton.style('padding', btnPadding);
    checkButton.style('border-radius', '6px');
    checkButton.style('cursor', 'pointer');
    checkButton.style('font-size', btnFontSize);
    checkButton.style('touch-action', 'manipulation');
    checkButton.elt.addEventListener('touchend', (e) => {
        e.preventDefault();
        checkAnswers();
    });
    
    scoreButton = createButton('Score: 0/10');
    scoreButton.parent(buttonContainer);
    scoreButton.mousePressed(showScoreFeedback);
    scoreButton.style('background-color', '#9C27B0');
    scoreButton.style('color', 'white');
    scoreButton.style('border', 'none');
    scoreButton.style('padding', btnPadding);
    scoreButton.style('border-radius', '6px');
    checkButton.style('cursor', 'pointer');
    scoreButton.style('font-size', btnFontSize);
    scoreButton.style('touch-action', 'manipulation');
    scoreButton.elt.addEventListener('touchend', (e) => {
        e.preventDefault();
        showScoreFeedback();
    });
    ...
    resetButton.style('padding', btnPadding);
    resetButton.style('border-radius', '6px');
    resetButton.style('cursor', 'pointer');
    resetButton.style('font-size', btnFontSize);
    resetButton.style('touch-action', 'manipulation');
    resetButton.elt.addEventListener('touchend', (e) => {
        e.preventDefault();
        resetGame();
    });
    ...
    infoButton.style('padding', btnPadding);
    infoButton.style('border-radius', '6px');
    infoButton.style('cursor', 'pointer');
    infoButton.style('font-size', btnFontSize);
    infoButton.style('touch-action', 'manipulation');
    infoButton.elt.addEventListener('touchend', (e) => {
        e.preventDefault();
        showInfo();
    });
    ...
    // Kortere labels voor mobiel
    if (IS_MOBILE) {
        checkButton.html('Check');
        scoreButton.html('0/10');
