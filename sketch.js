...
function setup() {
    ...
    let btnW = IS_MOBILE ? 65 : 130; //button breedte
    let btnH = IS_MOBILE ? 32 : 45;   //button hoogte
    let btnY = MARGIN + TITLE_SPACE + BUTTON_HEIGHT + 70; // button omhoog-omlaag
    let btnGap = IS_MOBILE ? 8 : 50; //button verder uiteen
    
    // Bereken totale breedte van alle buttons
    let totalWidth = btnW * 3 + (btnW + 20) + btnGap * 3; // 3 normale + 1 bredere (score)
    let startX = (width - totalWidth) / 2; // Gecentreerd
