...
function setup() {
    ...
    // Canvas container DAARNA
    let container = createDiv();
    container.parent(wrapper);
    container.style('position', 'relative');
    container.style('display', 'inline-block');
    container.style('order', '2'); // Komt na buttons
  ...
  // Button container EERST (komt boven canvas)
    let buttonContainer = createDiv();
    buttonContainer.parent(container); // ← Let op: parent is nu container, niet wrapper
    buttonContainer.style('display', 'flex');
    buttonContainer.style('gap', IS_MOBILE ? '4px' : '10px');
    buttonContainer.style('justify-content', 'center');
    buttonContainer.style('flex-wrap', 'wrap');
    buttonContainer.style('position', 'absolute');
    buttonContainer.style('top', (MARGIN + TITLE_SPACE + BUTTON_HEIGHT + 20) + 'px');
    buttonContainer.style('left', '50%');
    buttonContainer.style('transform', 'translateX(-50%)');
    buttonContainer.style('z-index', '100');
    buttonContainer.style('width', 'auto');
   
    let btnPadding = IS_MOBILE ? '4px 6px' : '10px 15px';
    let btnFontSize = IS_MOBILE ? '10px' : '16px';
